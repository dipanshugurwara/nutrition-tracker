...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: (error as Error).message || 'Failed to fetch entries' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: (error as Error).message || 'Failed to create entry' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!deleted) return NextResponse.json({ error: 'Failed to delete entry' }, { status: 500 });

    const summary = getDailySummary(entry.date);
    return NextResponse.json({ success: true, summary });
  } catch (error: unknown) {
    console.error('Error deleting entry:', error);
    return NextResponse.json({ error: (error as Error).message || 'Failed to delete entry' }, { status: 500 });
  }
}
